...
class LatestRatesCurrencyModel {
  String? date;
  String? base;
  Rates? rates;
  List? ratesList;
  List? rateValueList;
  LatestRatesCurrencyModel({this.date, this.base,this.ratesList, this.rates,this.rateValueList});

  LatestRatesCurrencyModel.fromJson(Map<String, dynamic> json) {
    date = json['date'];
    base = json['base'];
    rates = json['rates'] != null ? new Rates.fromJson(json['rates']) : null;
    ratesList = json['rates'].entries.map((entry) {
      return CurrencyRate(unit: entry.key, rate: entry.value.toString());
    }).toList();
    rateValueList = json['rates'].entries.map((entry) {
      return CurrencyRate(unit: entry.key, rate: entry.value.toString());
    }).toList();
  }
